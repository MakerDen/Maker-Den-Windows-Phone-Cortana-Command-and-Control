...
namespace Light {
    ...
    public sealed partial class App : Application {
        ...
        public void Initialise() {
            int retry = 0;
            while (mqtt == null && retry < 10) {
                try {
                    mqtt = new MqttClient("gloveboxAE.cloudapp.net");
                }
                catch { retry++; }
            }
        }
        ...
        public void Publish(string message) {
            ...
            Initialise();
            if (mqtt == null) { return; }
            while (!mqtt.IsConnected && retry < 10) {
                try {
                    mqtt.Connect("12345App");
                }
                catch { retry++; }
            }

            if (mqtt.IsConnected) { mqtt.Publish("gbcmd/all/" + message, encoding.GetBytes(" ")); }
        }
    }
}